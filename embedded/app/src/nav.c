...
{
    return (currVel - getMotionVel()) / USEC_TO_SEC((float)getTimersLastRetro() - (float)getTimersOldRetro());
}

/* Returns m/s */
static inline float velFromRetro(float currPos)
{
    return (currPos - getMotionPos()) / USEC_TO_SEC((float)getTimersLastRetro() - (float)getTimersOldRetro());
}
