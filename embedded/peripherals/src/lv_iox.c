...
static int setupIox()
{
    ...
    if (ret != 0)
        fprintf(stderr, "Error initializing LV IOX\n");
    return ret; /* TODO Add error checking */
}

int limSwitchGet(int limSwitch)
{
    if (limSwitch != PRIM_LIM_SWITCH && limSwitch != SEC_LIM_SWITCH && limSwitch != MCP_GPIOA_0 && limSwitch != MCP_GPIOA_2) {
