...
setInterval(function(){
  ...
  for (i=1; i< 6; i++){ //for loop that goes through each row of the table, 'i' being used to represent row (should probably change to r)
    ...
    if (y<Number(x[1].innerHTML)) { //if that obtained value is less than 2 (arbitrary number for now), style red and add to counter w which will be used to determine if any row had an error
      x[2].style.backgroundColor = '#FC6962';
      w++;
    }
    else if (y>Number(x[3].innerHTML)) { //if that obtained value is greater than 90 (arbitrary value for now), style green
